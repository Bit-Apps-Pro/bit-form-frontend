...
export const normalizeSelector = (selector) => selector
  ...
  .replace(/\\n\s*/g, '') // replace ",   " to "," and "\n   " to ""
  .replace(XRegExp('(?<=\\[)(\\s+)|(\\s+)(?=\\])', 'g'), '') // replace "[  " to "[" and  "  ]" to "]"
