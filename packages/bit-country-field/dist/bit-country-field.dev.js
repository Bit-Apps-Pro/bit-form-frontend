(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.bit_country_field = factory());
  ...
  class BitCountryField {
    ...
    #addEventListenersToElm() {
      this.#addEvent(this.#dropdownWrapperElm, 'click', e => {
        this.#handleDropdownClick(e);
      });
      this.#addEvent(this.#dropdownWrapperElm, 'keyup', e => {
        this.#handleDropdownClick(e);
      });
      this.#addEvent(this.#countryFieldWrapper, 'keydown', e => {
      ...
      this.#addEvent(this.#searchInputElm, 'keyup', e => {
        this.#handleSearchInput(e);
      });
      observeElement(this.#countryHiddenInputElm, 'value', (oldVal, newVal) => {
        this.#handleInputValueChange(oldVal, newVal);
      });
    }
    ...
    #detectCountryCodeFromIpAddress() {
      fetch('https://www.cloudflare.com/cdn-cgi/trace').then(resp => resp.text()).then(data => {
    ...
    #detectCountryCodeFromGeoLocation() {
      navigator.geolocation.getCurrentPosition(pos => {
        ...
        fetch(`http://api.geonames.org/countryCodeJSON?username=bitcodezoho1&lat=${latitude}&lng=${longitude}`).then(resp => resp.json()).then(data => {
    ...
    #detachAllEvents() {
      this.#allEventListeners.forEach(({
        selector,
        eventType,
        cb
      }) => {
