          ...
          <div className={css({ flx: 1, cg: 5 })}>
            ...
            {((msgType === 'snackbar' || ['slide-up', 'slide-down'].includes(animation)) && msgType !== 'below') && (
              <div className={css({ flx: 'align-center' })}>
                ...
                >
                  {
                    (positions[msgType][animation] || positions[msgType])?.map(value => <option key={`opt2${value}`} value={value}>{value.replace(/-/g, ' ').replace(/(^\w{1})|(\s{1}\w{1})/g, match => match.toUpperCase())}</option>)
                  }
                </select>
              </div>
            )}
