...
export default function MailChimpActions({ sheetConf, setSheetConf, formFields, address }) {
  ...
  return (
    ...
    <div className="pos-rel d-flx w-8">
      <TableCheckBox
        checked={sheetConf.actions?.address || false}
        onChange={(e) => actionHandler(e, 'address')}
        className="wdt-200 mt-4 mr-2"
        value="address"
        title={__('Add Address Field')}
        subTitle={__('Add Address Field')}
      />
      <TableCheckBox
        checked={sheetConf.actions?.double_opt_in || false}
      ...
      />
      <TableCheckBox
        checked={sheetConf.actions?.update || false}
        onChange={(e) => actionHandler(e, 'update')}
        className="wdt-200 mt-4 mr-2"
        value="user_share"
        title={__('Update Mail Chimp')}
        subTitle={__('Update Responses with MailChimp exist Aduience?')}
      />
    </div>
  )
}
