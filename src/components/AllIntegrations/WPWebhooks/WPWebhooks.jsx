...
function WPWebhooks({ formFields, setIntegration, integrations, allIntegURL }) {
  const history = useNavigate()
  const { formID } = useParams()
  const [step, setstep] = useState(1)
  const [snack, setSnackbar] = useState({ show: false })
  const [wpWebhooks, setWPWebhooks] = useState({
    name: 'WP Webhooks Web Hooks',
    type: 'WP Webhooks',
    method: 'POST',
    url: '',
  })
  ...
  return (
    <div>
      ...
      <TutorialLink
        title={tutorialLinks.wpWebhooks.title}
        youTubeLink={tutorialLinks.wpWebhooks.link}
      />
      {/* STEP 1 */}
      ...
      <div className="btcd-stp-page" style={{ width: step === 2 && 900, minHeight: step === 2 && `${900}px` }}>
        <WebHooksStepTwo
          step={step}
          saveConfig={() => saveIntegConfig(integrations, setIntegration, allIntegURL, wpWebhooks, history)}
        />
      </div>
    </div>
  )
