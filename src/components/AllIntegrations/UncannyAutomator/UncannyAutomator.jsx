...
function UncannyAutomator({ formFields, setIntegration, integrations, allIntegURL }) {
  const history = useNavigate()
  const { formID } = useParams()
  const [step, setstep] = useState(1)
  const [snack, setSnackbar] = useState({ show: false })
  const [uncannyAutomator, setUncannyAutomator] = useState({
    name: 'Uncanny Automator Web Hooks',
    type: 'Uncanny Automator',
    method: 'POST',
    url: '',
  })
  ...
  return (
    <div>
      ...
      <TutorialLink
        title={tutorialLinks.uncannyAutomator.title}
        youTubeLink={tutorialLinks.uncannyAutomator.link}
      />
      {/* STEP 1 */}
      ...
      <div className="btcd-stp-page" style={{ width: step === 2 && 900, minHeight: step === 2 && `${900}px` }}>
        <WebHooksStepTwo
          step={step}
          saveConfig={() => saveIntegConfig(integrations, setIntegration, allIntegURL, uncannyAutomator, history)}
        />
      </div>
    </div>
  )
