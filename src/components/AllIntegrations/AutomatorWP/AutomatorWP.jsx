...
function AutomatorWP({ formFields, setIntegration, integrations, allIntegURL }) {
  ...
  return (
    <div>
      ...
      <TutorialLink
        title={tutorialLinks.automatorwp.title}
        youTubeLink={tutorialLinks.automatorwp.link}
      />
      {/* STEP 1 */}
      ...
      <div className="btcd-stp-page" style={{ width: step === 2 && 900, minHeight: step === 2 && `${900}px` }}>
        <WebHooksStepTwo
          step={step}
          saveConfig={() => saveIntegConfig(integrations, setIntegration, allIntegURL, automatorwp, history)}
        />
      </div>
    </div>
  )
