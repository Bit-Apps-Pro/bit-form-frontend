...
export default function ZohoProjectsIntegLayout({
  ...
  return (
    <>
      ...
      <button
        onClick={() => refreshPortals(formID, projectsConf, setProjectsConf, setisLoading, setSnackbar)}
        className="icn-btn sh-sm ml-2 mr-2 tooltip"
        style={{ '--tooltip-txt': `'${__('Refresh Projects Portals')}'` }}
        type="button"
        disabled={isLoading}
      >
        &#x21BB;
      ...
      {(projectsConf?.event && projectsConf.event !== 'project') && (
        <>
          ...
          <button
            onClick={() => refreshProjects(formID, projectsConf, setProjectsConf, setisLoading, setSnackbar)}
            className="icn-btn sh-sm ml-2 mr-2 tooltip"
            style={{ '--tooltip-txt': `'${__('Refresh Projects')}'` }}
            type="button"
            disabled={isLoading}
          >
            &#x21BB;
          ...
          {!projectsConf.subEvent.includes('project') && (
            <>
              <select
                onChange={handleInput}
                name="milestoneId"
                value={projectsConf.milestoneId}
                className="btcd-paper-inp w-7"
              >
              ...
              <button
                onClick={() => refreshMilestones(formID, projectsConf, setProjectsConf, setisLoading, setSnackbar)}
                className="icn-btn sh-sm ml-2 mr-2 tooltip"
                style={{ '--tooltip-txt': `'${__('Refresh Milestones')}'` }}
                type="button"
                disabled={isLoading}
              >
                &#x21BB;
          ...
          {!projectsConf.subEvent.includes('project') && !projectsConf.subEvent.includes('milestone') && (
            <>
              <select
                onChange={handleInput}
                name="tasklistFlag"
                value={projectsConf.tasklistFlag}
                className="btcd-paper-inp w-7"
              >
              ...
              {(projectsConf?.tasklistFlag && !projectsConf?.subEvent.includes('milestone'))
                && (
                  <>
                    ...
                    <button
                      onClick={() => refreshTasklists(formID, projectsConf, setProjectsConf, setisLoading, setSnackbar)}
                      className="icn-btn sh-sm ml-2 mr-2 tooltip"
                      style={{ '--tooltip-txt': `'${__('Refresh Milestones')}'` }}
                      type="button"
                      disabled={isLoading}
        ...
        <>
          ...
          {(!projectsConf.subEvent.includes('project') && !projectsConf.subEvent.includes('milestone') && !projectsConf.subEvent.includes('tasklist')) && (
            <>
              ...
              <button
                onClick={() => refreshTasks(formID, projectsConf, setProjectsConf, setisLoading, setSnackbar)}
                className="icn-btn sh-sm ml-2 mr-2 tooltip"
                style={{ '--tooltip-txt': '"Refresh Tasks"' }}
                type="button"
                disabled={isLoading}
              >
                &#x21BB;
