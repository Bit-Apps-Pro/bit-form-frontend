    ...
    <div className={css(ut.mt2)}>
      ...
      <div className={css(s.segmentcontainer)}>
        ...
        {controller === 'Individual' && (
          <>
            <SizeControl min="0" inputHandler={handleValues} sizeHandler={({ unitKey, unitValue, id }) => handleValues({ value: unitValue, unit: unitKey, id })} id="0" label="T" width="100px" value={values[0] && getValue(values[0]) || 0} unit={values[0] && getUnit(values[0]) || 'px'} options={unitOption} />
            <SizeControl min="0" inputHandler={handleValues} sizeHandler={({ unitKey, unitValue, id }) => handleValues({ value: unitValue, unit: unitKey, id })} id="1" label="R" width="100px" value={values[1] && getValue(values[1]) || 0} unit={values[1] && getUnit(values[1]) || 'px'} options={unitOption} />
            <SizeControl min="0" inputHandler={handleValues} sizeHandler={({ unitKey, unitValue, id }) => handleValues({ value: unitValue, unit: unitKey, id })} id="2" label="B" width="100px" value={values[2] && getValue(values[2]) || 0} unit={values[2] && getUnit(values[2]) || 'px'} options={unitOption} />
            <SizeControl min="0" inputHandler={handleValues} sizeHandler={({ unitKey, unitValue, id }) => handleValues({ value: unitValue, unit: unitKey, id })} id="3" label="L" width="100px" value={values[3] && getValue(values[3]) || 0} unit={values[3] && getUnit(values[3]) || 'px'} options={unitOption} />
          </>
        )}
      </div>
    </div>
