...
type IconsUtilType = {
  ...
  onRemoveIcon?: (iconUrl: string) => void
}

function IconsUtil({
  selected = '', uploadLbl = '', onSaveIcon, onRemoveIcon,
}: IconsUtilType) {
  const [controller, setController] = useState({ parent: selected || 'Icons' })
  const [files, setFiles] = useState([])
  const [loading, setLoading] = useState(false)
  const [dnLoading, setDnLoading] = useState(false)
  const [selectedIcon, setSelectedIcon] = useState<{ id: number, url: string } | {}>({})
  ...
  const saveIcn = () => {
    setDnLoading(true)
    bitsFetch({ id: selectedIcon.id, src: selectedIcon.url }, 'bitforms_icn_save_setting')
      .then(res => {
        if (res !== undefined && res.success) {
          onSaveIcon(res.data)
        }
  ...
  const delIcon = file => {
    ...
    if (onRemoveIcon) onRemoveIcon(file)
  }
  ...
  const selectedSaveIcon = () => {
    onSaveIcon(selectedIcon.url)
  }

  const handlePrefixIcon = (id, iconUrl) => {
    setSelectedIcon({ id, url: iconUrl })
  }
          ...
          <div className={css(ut.flxc, ut.mt2, s.icon)}>
            {icons.map((item) => (
              <button
                ...
                className={`${css(s.icnBtn)} ${url + item.url === selectedIcon.url && css(s.active, s.activeIcn)}`}
              ...
              {!!files.length && files.map((file, index) => (
                ...
                >
                  ...
                  <button
                    ...
                    title={file.name}
                    className={`${css(s.icnBtn)} ${`${bits.iconURL}/${file}` === selectedIcon.url && css(s.active)}`}
                  >
