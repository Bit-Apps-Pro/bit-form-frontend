...
export default function SelectUtil({
  ...
  const handleOptionClick = (e: MouseEvent<HTMLButtonElement> | KeyboardEvent<HTMLButtonElement>, i: number) => {
    e.stopPropagation()
  ...
  const handleDropdownClick = (e: MouseEvent<HTMLButtonElement> | KeyboardEvent<HTMLButtonElement>) => {
    if (e.type === 'click') {
      if (menu.open) {
        setMenu({ height: 0, open: false })
      } else if (optionWrpRef.current) {
...
type OptionType = { label: string, value: ValueType, icn?: string }
