...
export default function StripeQuickTweaks() {
  ...
  const getPropValue = (prop = 'width') => {
    let brsValue = getValueByObjPath(styles, propertyPath('stripe-btn', prop))
  ...
  const setBtnSize = (elementKey, value) => {
    const btnSizeValues = {
      'small-2': ['12px', '7px 10px', '14px', '14px'],
      'small-1': ['14px', '9px 15px', '16px', '16px'],
      medium: ['16px', '11px 20px', '18px', '18px'],
      'large-1': ['18px', '12px 22px', '20px', '20px'],
      'large-2': ['21px', '14px 24px', '24px', '24px'],
    }
    console.log('setBtnSize', elementKey, value)
    const propertyPaths = [
      propertyPath(elementKey, 'font-size'),
      propertyPath(elementKey, 'padding'),
      propertyPath('stripe-icn', 'width'),
    ...
    <>
      <ThemeStylePropertyBlock label="Size">
        <select
          ...
          value={fieldSize}
          onChange={e => setBtnSize('stripe-btn', e.target.value)}
